...
const COLORS = ['#0E47AB', '#25D366', '#6B46C1', '#F05537', '#1E6F3E']
        ...
        <div className="container">
          <Reveal>
            ...
            <h1 style={{ fontSize: 88, marginTop: 24, lineHeight: 0.9, color: 'white', maxWidth: 900 }}>
              Society vs<br /><span style={{ color: '#D4A75B' }}>les alternatives.</span>
            </h1>
            <p style={{ fontSize: 20, marginTop: 28, maxWidth: 640, lineHeight: 1.6, color: 'rgba(255,255,255,0.75)' }}>
              Pourquoi des dizaines de milliers de leaders communautaires africains migrent vers Society — et ce qu'ils laissent derrière.
            </p>
          </Reveal>
        </div>
      </section>
      ...
      <section style={{ padding: '80px 64px 120px' }}>
        <div className="container">
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 700 }}>
                  <thead>
                    <tr>
                      ...
                      {COLS.map((col, i) => (
                        <th key={col} style={{ padding: '24px 20px', textAlign: 'center', fontSize: 14, fontWeight: 800, background: i === 0 ? '#0E47AB' : '#F8F8F8', color: i === 0 ? 'white' : '#0a0a0a', borderBottom: '1px solid #E5E5E5', minWidth: 110 }}>
                  ...
                  <tbody>
                    {ROWS.map((row, ri) => (
                      <tr key={ri} style={{ borderTop: '1px solid #E5E5E5' }}>
                        <td style={{ padding: '18px 28px', fontSize: 14, fontWeight: 600, color: '#0a0a0a' }}>{row[0]}</td>
                        {row.slice(1).map((v, ci) => (
                          <td key={ci} style={{ padding: 18, textAlign: 'center', background: ci === 0 ? '#F4F7FE' : 'transparent' }}>
                            {v
                              ? <span style={{ display: 'inline-grid', placeItems: 'center', width: 28, height: 28, borderRadius: '50%', background: ci === 0 ? '#0E47AB' : '#DCFCE7', color: ci === 0 ? 'white' : '#166534', fontSize: 14, fontWeight: 800 }}>✓</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24, marginTop: 64 }}>
            {[
              { icon: 'money', color: '#0E47AB', bg: '#E8EFFC', t: 'Mobile Money natif', d: 'Orange Money, Wave, MTN MoMo — intégrés nativement, sans passerelle tierce, sans frais cachés.' },
              { icon: 'globe', color: '#166534', bg: '#DCFCE7', t: 'Conçu pour l\'Afrique', d: 'Serveurs en Afrique, support francophone, adapté aux usages offline-first et aux contraintes réseau.' },
              { icon: 'shield', color: '#D4A75B', bg: '#F6EDD8', t: 'Tout-en-un souverain', d: 'Vos données vous appartiennent. Pas d\'algorithme tiers, pas de dépendance à une plateforme externe.' },
            ].map((c, i) => (
              <Reveal key={i} delay={i * 100}>
                <div className="hover-lift" style={{ padding: 36, background: '#F8F8F8', borderRadius: 24 }}>
                  ...
                  <p style={{ fontSize: 15, color: '#3A3A3A', lineHeight: 1.65 }}>{c.d}</p>
                </div>
              </Reveal>
            ))}
          </div>
        ...
        <div className="container" style={{ textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 16, justifyContent: 'center', marginTop: 40 }}>
            <NavLink to="/inscription" className="btn btn-gold btn-xl">
              Commencer gratuitement <Icon name="arrow" size={16} color="#0a0a0a" />
            </NavLink>
