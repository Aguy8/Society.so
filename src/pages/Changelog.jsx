...
const RELEASES = [
  ...
  },
  {
    ...
    items: [
      ...
      { type: '⚡ Amélioré', t: 'Onboarding guidé pour les nouveaux fondateurs' },
...
const TYPE_COLORS = {
  '✨ Nouveau': { bg: '#E8EFFC', c: '#0E47AB' },
  '⚡ Amélioré': { bg: '#F6EDD8', c: '#8B6020' },
  '🐛 Corrigé': { bg: '#DCFCE7', c: '#166534' },
...
export default function Changelog() {
  return (
    <div className="page">
      ...
      <section style={{ padding: '120px 64px 80px', background: '#F8F8F8' }}>
        <div className="container">
          ...
          <h1 style={{ fontSize: 100, marginTop: 28, lineHeight: 0.9, maxWidth: 800 }}>
            Ce qu'on<br />
            <span style={{ color: '#0E47AB' }}>améliore</span><br />
            chaque semaine.
          </h1>
          <p style={{ fontSize: 20, marginTop: 32, maxWidth: 600, lineHeight: 1.5 }}>
            Society évolue en permanence. Voici l'historique complet de toutes les mises à jour, nouvelles fonctionnalités et corrections.
          </p>
        </div>
      </section>
                  ...
                  <Reveal key={i} delay={i * 80}>
                    <div style={{ display: 'grid', gridTemplateColumns: '48px 1fr', gap: 28 }}>
                      <div style={{ width: 48, height: 48, borderRadius: '50%', background: r.tagBg, display: 'grid', placeItems: 'center', border: '3px solid white', boxShadow: '0 0 0 2px ' + r.tagColor + '30', flexShrink: 0, zIndex: 1 }}>
                        <Icon name="bolt" size={20} color={r.tagColor} />
                      </div>
                      <div>
                        ...
                        <h2 style={{ fontSize: 22, fontWeight: 800, marginBottom: 20, color: '#0a0a0a' }}>{r.titre}</h2>
                          ...
                          {r.items.map((item, j) => {
                            ...
                            return (
                              <div key={j} style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                                ...
                                <span style={{ fontSize: 14, color: '#3A3A3A', lineHeight: 1.5, marginTop: 2 }}>{item.t}</span>
                              </div>
                            )
                          })}
                        </div>
      ...
      <section style={{ padding: '64px 64px 100px', background: '#0E47AB' }}>
        <div className="container" style={{ textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', maxWidth: 480, margin: '0 auto', flexWrap: 'wrap' }}>
            ...
            <button className="btn btn-gold">Je m'abonne</button>
          </div>
        </div>
      </section>
