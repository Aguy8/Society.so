        ...
        <div className="container">
          ...
          {stats && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20, marginBottom: 48 }}>
              {[
                { v: stats.total_communities, l: 'Communautés', color: '#0E47AB' },
                { v: stats.total_membres, l: 'Membres au total', color: '#D4A75B' },
                { v: stats.by_type?.length || 0, l: 'Types actifs', color: '#0F8B4F' },
                { v: stats.by_pays?.[0]?.pays || '—', l: 'Pays principal', color: '#5C2E91' },
              ].map((s, i) => (
                ...
                <div style={{ display: 'grid', gap: 12 }}>
                  {communities.map(c => (
                    <div key={c.id} onClick={() => selectCommunity(c)}
                      style={{ padding: 20, background: selected?.id === c.id ? '#F4F7FE' : 'white', border: `1.5px solid ${selected?.id === c.id ? '#0E47AB' : '#E5E5E5'}`, borderRadius: 16, cursor: 'pointer', display: 'flex', gap: 16, alignItems: 'center' }}>
                      <div style={{ width: 44, height: 44, borderRadius: 12, background: c.couleur || '#0E47AB', flexShrink: 0 }} />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
                          <span style={{ padding: '2px 8px', background: '#E8EFFC', color: '#0E47AB', borderRadius: 999, fontSize: 11, fontWeight: 600 }}>Plan {c.plan}</span>
                      ...
                      <button onClick={ev => { ev.stopPropagation(); deleteCommunity(c.id) }}
                        style={{ padding: '6px 12px', background: '#FFF5F5', color: '#C23B5A', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                        Suppr.
                      </button>
                    </div>
                  ))}
              ...
              <div>
                <div style={{ padding: 24, background: 'white', borderRadius: 20, border: '1px solid #E5E5E5', marginBottom: 20 }}>
                  <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                    <div style={{ width: 52, height: 52, borderRadius: 14, background: selected.couleur || '#0E47AB', flexShrink: 0 }} />
                    <div>
                  ...
                  {selected.description && (
                    <p style={{ fontSize: 14, color: '#3A3A3A', marginTop: 16, lineHeight: 1.5, marginBottom: 0 }}>{selected.description}</p>
                  )}
                </div>
                ...
                {showAddMember && (
                  <form onSubmit={addMember} style={{ padding: 24, background: '#F8F8F8', borderRadius: 16, marginBottom: 16, display: 'grid', gap: 14 }}>
                    ...
                    {memberError && <div style={{ fontSize: 13, color: '#C23B5A' }}>{memberError}</div>}
                  ...
                  <div style={{ display: 'grid', gap: 10 }}>
                    {members.map(m => (
                      <div key={m.id} style={{ padding: '16px 20px', background: 'white', border: '1px solid #E5E5E5', borderRadius: 14, display: 'flex', alignItems: 'center', gap: 14 }}>
                        <div style={{ width: 40, height: 40, borderRadius: '50%', background: '#E8EFFC', display: 'grid', placeItems: 'center', fontWeight: 700, fontSize: 14, color: '#0E47AB', flexShrink: 0 }}>
                        ...
                        <button onClick={() => deleteMember(m.id)}
                          style={{ padding: '4px 10px', background: '#FFF5F5', color: '#C23B5A', border: 'none', borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: 'pointer', flexShrink: 0 }}>
                          ×
                        </button>
                      </div>
                    ))}
