  ...
  return (
    <div className="page">
      ...
      <section style={{ padding: '120px 64px 80px', background: '#0a0a0a', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundImage: 'radial-gradient(circle at 20% 50%, rgba(14,71,171,0.25) 0%, transparent 60%), radial-gradient(circle at 80% 20%, rgba(212,167,91,0.1) 0%, transparent 50%)' }} />
        <div className="container" style={{ position: 'relative', zIndex: 2, textAlign: 'center' }}>
          <span className="eyebrow white">Documentation</span>
          <h1 style={{ fontSize: 80, marginTop: 24, lineHeight: 0.92, color: 'white', maxWidth: 800, margin: '24px auto 0' }}>
            Construisez avec<br />
            <span style={{ color: '#D4A75B' }}>Society API.</span>
          </h1>
          <p style={{ fontSize: 18, marginTop: 28, maxWidth: 560, lineHeight: 1.55, color: 'rgba(255,255,255,0.7)', margin: '28px auto 0' }}>
            Documentation complète, exemples de code et SDKs pour intégrer Society dans vos applications en quelques heures.
          </p>
          <div style={{ marginTop: 48, maxWidth: 600, margin: '48px auto 0', position: 'relative' }}>
            <div style={{ position: 'absolute', left: 20, top: '50%', transform: 'translateY(-50%)', opacity: 0.5 }}>
            ...
            <input
              ...
              style={{ width: '100%', padding: '18px 24px 18px 52px', borderRadius: 16, border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(255,255,255,0.08)', color: 'white', fontSize: 15, outline: 'none', boxSizing: 'border-box' }}
            />
          </div>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 20, flexWrap: 'wrap' }}>
            {['Démarrage rapide', 'Authentification', 'Webhooks', 'Mobile Money', 'SDK'].map(tag => (
              <button key={tag} style={{ padding: '8px 16px', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 999, color: 'rgba(255,255,255,0.7)', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                {tag}
              </button>
            ))}
          </div>
      ...
      <section style={{ padding: '80px 64px' }}>
        <div className="container">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 40 }}>
            ...
            <a href="https://github.com/DiferoCorp/society-sdk" target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 24px', background: '#0a0a0a', color: 'white', borderRadius: 12, textDecoration: 'none', fontWeight: 700, fontSize: 14 }}>
            ...
            <div style={{ display: 'grid', gap: 16 }}>
              {updates.map((u, i) => (
                <div key={i} style={{ padding: '20px 24px', background: 'white', borderRadius: 16, border: '1px solid #EBEBEB', display: 'grid', gridTemplateColumns: '100px 1fr', gap: 16, alignItems: 'start' }}>
                  <div>
                    <div style={{ fontFamily: 'monospace', fontSize: 13, fontWeight: 700, color: '#0a0a0a' }}>{u.version}</div>
                  ...
                  <p style={{ fontSize: 14, color: '#3A3A3A', lineHeight: 1.5 }}>{u.desc}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section style={{ padding: '64px 64px 80px' }}>
        <div className="container">
          <div style={{ background: '#0E47AB', borderRadius: 28, padding: '56px 64px', display: 'grid', gridTemplateColumns: '1fr auto', gap: 48, alignItems: 'center' }}>
            <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, minWidth: 220 }}>
              <a href="https://github.com/DiferoCorp" target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 22px', background: 'white', color: '#0a0a0a', borderRadius: 12, textDecoration: 'none', fontWeight: 700, fontSize: 14 }}>
                <Icon name="globe" size={16} color="#0a0a0a" />
                GitHub · DiferoCorp
              </a>
