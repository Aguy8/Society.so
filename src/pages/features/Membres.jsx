...
const FEATURES = [
  ...
  ['Onboarding automatique', 'Les nouveaux membres reçoivent une invitation personnalisée et un guide de bienvenue.'],
]
...
export default function FeatureMembres() {
  return (
    <div className="page">
      ...
      <section style={{ padding: '120px 64px 80px', background: '#0E47AB' }}>
        <div className="container">
          ...
          <h1 style={{ fontSize: 88, lineHeight: 0.9, color: 'white', maxWidth: 800 }}>
            Gestion<br /><span style={{ color: '#D4A75B' }}>membres.</span>
          </h1>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 20 }}>
                      {[['1 248', 'Membres'], ['94 %', 'À jour'], ['87 %', 'Actifs']].map(([v, l], i) => (
                        <div key={i} style={{ padding: 14, background: '#F8F8F8', borderRadius: 12, textAlign: 'center' }}>
                          <div style={{ fontSize: 20, fontWeight: 800, color: '#0E47AB' }}>{v}</div>
                          <div style={{ fontSize: 11, color: '#6B6B6B', marginTop: 2 }}>{l}</div>
                        </div>
                      ))}
                    </div>
                    {[
                      { n: 'Awa K.', r: 'Trésorière · À jour', img: 'https://images.unsplash.com/photo-1531123897727-8f11ebb3a745?auto=format&fit=crop&w=400&q=80', ok: true },
                      { n: 'Mamadou D.', r: 'Bénévole · À jour', img: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=400&q=80', ok: true },
                      { n: 'Ibrahim S.', r: 'Membre · En retard', img: 'https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?auto=format&fit=crop&w=400&q=80', ok: false },
                    ].map((m, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderTop: '1px solid #F2F2F2' }}>
                        ...
                        <div style={{ padding: '4px 10px', background: m.ok ? '#E8EFFC' : '#FEE2E2', color: m.ok ? '#0E47AB' : '#991B1B', borderRadius: 999, fontSize: 11, fontWeight: 600 }}>{m.ok ? 'À jour' : 'Relancer'}</div>
                      </div>
                    ))}
                  </div>
                </div>
