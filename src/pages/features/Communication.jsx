      ...
      <section style={{ padding: '120px 64px 80px', background: '#F8F8F8' }}>
        <div className="container">
          ...
          <div style={{ display: 'inline-flex', padding: '8px 16px', background: '#E8EFFC', borderRadius: 999, marginBottom: 24 }}>
            <Icon name="chat" size={18} color="#0E47AB" />
            <span style={{ color: '#0E47AB', fontWeight: 700, fontSize: 13, marginLeft: 8 }}>COMMUNICATION</span>
          </div>
          <h1 style={{ fontSize: 88, lineHeight: 0.9, maxWidth: 900 }}>
            Communiquez<br /><span style={{ color: '#0E47AB' }}>mieux.</span><br />
            Pas plus fort.
          </h1>
          <p style={{ fontSize: 20, marginTop: 28, maxWidth: 580, lineHeight: 1.6, color: '#3A3A3A' }}>
      ...
      <section style={{ padding: '80px 64px 120px' }}>
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24, marginBottom: 80 }}>
            {[
              { icon: 'chat', color: '#0E47AB', bg: '#E8EFFC', t: 'Canaux par sous-groupe', d: 'Créez des canaux de communication distincts par comité, région, thématique — sans pollution croisée.' },
              { icon: 'bolt', color: '#D4A75B', bg: '#F6EDD8', t: 'Messages ciblés', d: 'Envoyez un message uniquement aux trésoriers, aux bénévoles actifs, ou aux membres en retard de cotisation.' },
              { icon: 'bell', color: '#166534', bg: '#DCFCE7', t: 'Notifications push', d: 'Vos membres reçoivent une notification sur leur téléphone. Taux d\'ouverture 4x supérieur à l\'email.' },
              { icon: 'users', color: '#5C2E91', bg: '#F3F0FF', t: 'Annonces & newsletters', d: 'Envoyez une newsletter hebdomadaire automatique avec les actualités, événements et cotisations.' },
              ...
              { icon: 'shield', color: '#0a0a0a', bg: '#F2F2F2', t: 'Historique & archivage', d: 'Toutes les communications sont archivées et consultables. Plus aucune information perdue.' },
            ].map((f, i) => (
          ...
          <Reveal>
            <div style={{ background: '#0a0a0a', borderRadius: 32, padding: '64px 80px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#D4A75B', marginBottom: 16 }}>● LE VRAI PROBLÈME</div>
                  ...
                  {[
                    ...
                    <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                      <div style={{ padding: '14px 16px', background: 'rgba(255,255,255,0.04)', borderRadius: 12, fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>
                        <span style={{ color: '#C23B5A', marginRight: 8 }}>✕</span>{r.pb}
                      </div>
                      <div style={{ padding: '14px 16px', background: 'rgba(14,71,171,0.2)', borderRadius: 12, fontSize: 13, color: 'rgba(255,255,255,0.85)' }}>
                ...
                {[
                  ...
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 0', borderTop: i ? '1px solid rgba(255,255,255,0.06)' : 'none' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      {c.unread > 0 && <div style={{ background: '#0E47AB', color: 'white', borderRadius: 999, fontSize: 10, fontWeight: 800, padding: '2px 6px', marginTop: 4, display: 'inline-block' }}>{c.unread}</div>}
                    </div>
                  </div>
                ))}
              </div>
