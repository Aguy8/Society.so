      ...
      <section style={{ padding: '120px 64px 80px', background: '#0a0a0a' }}>
        <div className="container">
          ...
          <div style={{ display: 'inline-flex', padding: '8px 16px', background: '#D4A75B22', border: '1px solid #D4A75B44', borderRadius: 999, marginBottom: 24 }}>
            <Icon name="money" size={18} color="#D4A75B" />
            <span style={{ color: '#D4A75B', fontWeight: 700, fontSize: 13, marginLeft: 8 }}>MODULE 02</span>
          </div>
          <h1 style={{ fontSize: 88, lineHeight: 0.9, color: 'white', maxWidth: 900 }}>
            Monétisation<br /><span style={{ color: '#D4A75B' }}>& Finances.</span>
          </h1>
      ...
      <section style={{ padding: '80px 64px' }}>
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
            {[
              { icon: 'money', color: '#D4A75B', bg: '#F6EDD8', t: 'Cagnottes en 3 clics', d: 'Lancez une collecte en moins de 2 minutes. Lien partageable, suivi en temps réel, notifications automatiques.' },
              { icon: 'calendar', color: '#0E47AB', bg: '#E8EFFC', t: 'Cotisations automatisées', d: 'Relances SMS, rappels par email, historique — plus aucun conflit de trésorerie.' },
              { icon: 'star', color: '#166534', bg: '#DCFCE7', t: 'Abonnements premium', d: 'Contenus exclusifs, accès VIP, espaces membres payants — monétisez votre expertise.' },
              { icon: 'bolt', color: '#9D174D', bg: '#FDF2F8', t: 'Billetterie + QR codes', d: 'Événements physiques ou virtuels, vente de billets, vérification à l\'entrée.' },
              { icon: 'chart', color: '#5C2E91', bg: '#F3F0FF', t: 'Crowdfunding participatif', d: 'Projets à financer avec objectif, durée, paliers de récompenses pour les donateurs.' },
              { icon: 'shield', color: '#0a0a0a', bg: '#F2F2F2', t: 'Reporting financier', d: 'Exports comptables, reçus automatiques, transparence totale pour les bailleurs.' },
            ].map((f, i) => (
              <Reveal key={i} delay={i * 80} direction="scale">
                <div className="hover-lift" style={{ padding: 32, background: '#F8F8F8', borderRadius: 24 }}>
                  ...
                  <p style={{ fontSize: 14, color: '#3A3A3A', lineHeight: 1.65 }}>{f.d}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section style={{ padding: '80px 64px', background: '#D4A75B' }}>
        <div className="container">
          <Reveal style={{ textAlign: 'center', marginBottom: 48 }}>
            <h2 style={{ fontSize: 48, color: '#0a0a0a', fontWeight: 800 }}>Compatible avec tous vos paiements.</h2>
          </Reveal>
          <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
            {['🟠 Orange Money', '🔵 Wave', '🟡 MTN MoMo', '🔷 Flooz', '💳 Visa / Mastercard', '🏦 Virement bancaire'].map((p, i) => (
              <div key={i} style={{ padding: '14px 24px', background: 'rgba(0,0,0,0.1)', borderRadius: 14, fontSize: 15, fontWeight: 700, color: '#0a0a0a' }}>{p}</div>
            ))}
          </div>
        </div>
      </section>
        ...
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
            <Reveal direction="left">
              ...
              <h2 style={{ fontSize: 56, marginTop: 20 }}>Des chiffres qui<br />parlent d'eux-mêmes.</h2>
              <div style={{ display: 'grid', gap: 16, marginTop: 40 }}>
                {[
                  { n: '28 → 4 %', l: 'Taux d\'impayés moyen après Society', color: '#0E47AB' },
                  { n: '6h → 30min', l: 'Temps de comptabilité par semaine', color: '#D4A75B' },
                  { n: '0', l: 'Conflit financier rapporté chez nos utilisateurs', color: '#166534' },
                ].map((s, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 20, padding: 24, background: '#F8F8F8', borderRadius: 16 }}>
                    ...
                    <div style={{ fontSize: 15, color: '#3A3A3A' }}>{s.l}</div>
                  </div>
                ))}
              </div>
            </Reveal>
            <Reveal direction="right">
              <div style={{ background: 'white', borderRadius: 28, padding: 32, boxShadow: '0 20px 60px rgba(0,0,0,0.08)', border: '1px solid #E5E5E5' }}>
                ...
                <div style={{ height: 8, background: '#F2F2F2', borderRadius: 999, marginTop: 8, overflow: 'hidden' }}>
                  <div style={{ width: '72%', height: '100%', background: 'linear-gradient(90deg, #0E47AB, #3B7FFF)' }} />
                </div>
