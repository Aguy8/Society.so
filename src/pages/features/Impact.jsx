      ...
      <section style={{ padding: '120px 64px 80px', background: '#0E47AB', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="container" style={{ position: 'relative', zIndex: 2 }}>
          ...
          <div style={{ display: 'inline-flex', padding: '8px 16px', background: 'rgba(255,255,255,0.12)', borderRadius: 999, marginBottom: 24 }}>
            <Icon name="chart" size={18} color="#D4A75B" />
            <span style={{ color: '#D4A75B', fontWeight: 700, fontSize: 13, marginLeft: 8 }}>VISIBILITÉ & IMPACT</span>
          </div>
          <h1 style={{ fontSize: 88, lineHeight: 0.9, color: 'white', maxWidth: 900 }}>
            Prouvez votre<br /><span style={{ color: '#D4A75B' }}>impact.</span><br />
            Automatiquement.
          </h1>
          <p style={{ fontSize: 20, marginTop: 28, maxWidth: 600, lineHeight: 1.6, color: 'rgba(255,255,255,0.8)' }}>
            Transformez chaque action de votre communauté en preuve mesurable. Pages publiques, rapports PDF, indicateurs ODD — pour convaincre les bailleurs et attirer les partenaires.
          </p>
        </div>
      </section>
      ...
      <section style={{ padding: '80px 64px' }}>
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 24, marginBottom: 80 }}>
            {[
              { n: 14290, suffix: '', prefix: '', label: 'Bénéficiaires touchés en moyenne', color: '#0E47AB' },
              { n: 62, suffix: ' M', prefix: '', label: 'FCFA collectés en moyenne/an', color: '#D4A75B' },
              { n: 10, suffix: ' min', prefix: '', label: 'Pour générer un rapport PDF complet', color: '#166534' },
              ...
              <Reveal key={i} delay={i * 100} direction="scale">
                <div style={{ padding: 32, background: '#F8F8F8', borderRadius: 24, textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 14, color: '#3A3A3A', marginTop: 8, lineHeight: 1.5 }}>{s.label}</div>
                </div>
              </Reveal>
            ))}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center', marginBottom: 80 }}>
            <Reveal direction="left">
              ...
              <p style={{ fontSize: 17, marginTop: 20, lineHeight: 1.6, color: '#3A3A3A' }}>
              ...
              <div style={{ marginTop: 32, display: 'grid', gap: 12 }}>
                {['URL personnalisée society.ci/votre-communaute', 'Chiffres mis à jour en temps réel', 'Témoignages des bénéficiaires', 'Galerie de photos et vidéos'].map((c, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 15, color: '#0a0a0a' }}>
                    <span style={{ color: '#0E47AB', fontWeight: 800 }}>✓</span> {c}
                  </div>
                ))}
              </div>
            </Reveal>
            <Reveal direction="right">
              <div style={{ padding: 40, background: '#0E47AB', borderRadius: 28, color: 'white' }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.6)', letterSpacing: '0.06em' }}>PAGE D'IMPACT — society.ci/cansein</div>
                ...
                <div style={{ marginTop: 16, padding: 16, background: 'rgba(255,255,255,0.06)', borderRadius: 12 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {['ODD 1', 'ODD 4', 'ODD 8', 'ODD 17'].map(odd => (
                      <span key={odd} style={{ padding: '4px 10px', background: 'rgba(212,167,91,0.2)', borderRadius: 999, fontSize: 11, fontWeight: 700, color: '#D4A75B' }}>{odd}</span>
                    ))}
                  </div>
                </div>
                <button style={{ marginTop: 16, width: '100%', padding: 14, background: '#D4A75B', color: '#0a0a0a', border: 'none', borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
          ...
          <Reveal>
            <div style={{ background: '#F8F8F8', borderRadius: 28, padding: '56px 64px' }}>
              <div style={{ textAlign: 'center', marginBottom: 48 }}>
                <span className="eyebrow">Rapports bailleurs</span>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
                {[
                  ...
                  { n: '03', t: 'Téléchargez le PDF', d: 'Un rapport professionnel, prêt à envoyer à vos bailleurs et partenaires.' },
                ].map((s, i) => (
                  <div key={i} style={{ padding: 28, background: 'white', borderRadius: 20, border: '1px solid #E5E5E5' }}>
                    ...
                    <p style={{ fontSize: 14, color: '#3A3A3A', marginTop: 8, lineHeight: 1.6 }}>{s.d}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section style={{ padding: '80px 64px 120px', background: '#0a0a0a' }}>
        <div className="container" style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: 56, color: 'white', fontWeight: 800, maxWidth: 700, margin: '0 auto' }}>
            Votre impact mérite<br />d'être <span style={{ color: '#D4A75B' }}>visible</span>.
          </h2>
          <NavLink to="/inscription" className="btn btn-gold btn-xl" style={{ marginTop: 40, display: 'inline-flex' }}>
            Créer ma page d'impact <Icon name="arrow" size={16} color="#0a0a0a" />
          </NavLink>
        </div>
      </section>
