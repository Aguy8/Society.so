    ...
    <div className="page">
      ...
      <section style={{ padding: '120px 64px 80px', background: '#0E47AB', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="container" style={{ position: 'relative', zIndex: 2 }}>
          ...
          <h1 style={{ fontSize: 80, lineHeight: 0.92, color: 'white', maxWidth: 880 }}>
            Vos données<br />
            <span style={{ color: '#D4A75B' }}>méritent</span><br />
            la meilleure protection.
          </h1>
          <p style={{ fontSize: 19, marginTop: 32, maxWidth: 620, lineHeight: 1.55, color: 'rgba(255,255,255,0.8)' }}>
            La confiance est au cœur de Society. Nous appliquons les standards de sécurité les plus élevés pour protéger chaque membre, chaque transaction et chaque donnée communautaire.
          </p>
            ...
            {pilliers.map((p, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0, border: '1px solid #EBEBEB', borderRadius: 28, overflow: 'hidden' }}>
                <div style={{ padding: '56px 56px', background: p.color }}>
                  ...
                  <p style={{ fontSize: 16, marginTop: 20, lineHeight: 1.65, color: '#3A3A3A' }}>{p.description}</p>
                </div>
                  ...
                  <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'grid', gap: 16 }}>
                    {p.details.map((d, j) => (
                      <li key={j} style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                        ...
                        <span style={{ fontSize: 15, lineHeight: 1.5, color: '#0a0a0a' }}>{d}</span>
                      </li>
                    ))}
                  </ul>
                </div>
      ...
      <section style={{ padding: '64px 64px', background: '#F8F8F8' }}>
        <div className="container">
          <div className="two-col-grid">
            <div style={{ padding: '48px', background: '#0a0a0a', borderRadius: 28, color: 'white' }}>
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: '0.06em', color: '#D4A75B' }}>● DONNÉES EN AFRIQUE</div>
              ...
              <p style={{ fontSize: 16, color: 'rgba(255,255,255,0.75)', marginTop: 20, lineHeight: 1.65 }}>
                Par défaut, les données de toutes les communautés Society sont hébergées dans des datacenters situés en Afrique de l'Ouest. Aucun transfert de données vers des serveurs hors continent sans votre consentement explicite.
              </p>
              <div style={{ display: 'grid', gap: 12, marginTop: 32 }}>
                {['Datacenter principal : Abidjan, CI', 'Réplication : Lagos, NG (backup)', 'Aucun transfert Europe / USA sans consentement', 'Conformité CEDEAO et loi ivoirienne n° 2013-450'].map((item, i) => (
                  <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                    <div style={{ width: 6, height: 6, borderRadius: 999, background: '#D4A75B', flexShrink: 0 }} />
            ...
            <div style={{ padding: '48px', background: 'white', borderRadius: 28, border: '1px solid #EBEBEB' }}>
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: '0.06em', color: '#0E47AB' }}>● DIVULGATION RESPONSABLE</div>
              <h2 style={{ fontSize: 36, marginTop: 16, fontWeight: 800, lineHeight: 1.2 }}>Vous avez trouvé une faille ?</h2>
              <p style={{ fontSize: 16, color: '#3A3A3A', marginTop: 20, lineHeight: 1.65 }}>
                Nous accueillons positivement les signalements de vulnérabilités. Notre programme de divulgation responsable récompense les chercheurs de 50 000 à 500 000 FCFA selon la criticité.
              </p>
              <div style={{ padding: '20px 24px', background: '#F8F8F8', borderRadius: 16, marginTop: 28, fontFamily: 'monospace', fontSize: 14, color: '#0a0a0a' }}>
                security@society.ci
              </div>
      ...
      <section style={{ padding: '80px 64px 120px' }}>
        <div className="container" style={{ textAlign: 'center' }}>
          <Icon name="shield" size={48} color="#0E47AB" />
          ...
          <p style={{ fontSize: 17, color: '#6B6B6B', marginTop: 16, maxWidth: 520, margin: '16px auto 0', lineHeight: 1.6 }}>
            Notre équipe sécurité est disponible pour répondre à toute demande des entreprises, institutions et administrateurs de grandes communautés.
          </p>
