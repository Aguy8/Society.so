            ...
            {plans.map((p, i) => {
              ...
              return (
                <div key={i} style={{ padding: 32, background: p.color, color: p.text, borderRadius: 28, position: 'relative', display: 'flex', flexDirection: 'column' }}>
                  ...
                  <NavLink to="/inscription" style={{ marginTop: 24, padding: '14px 20px', textAlign: 'center', borderRadius: 999, fontSize: 14, fontWeight: 700, textDecoration: 'none', background: p.text === 'white' ? 'white' : '#0a0a0a', color: p.text === 'white' ? p.color : 'white', display: 'block' }}>{p.cta}</NavLink>
                </div>
              )
            })}
          </div>
